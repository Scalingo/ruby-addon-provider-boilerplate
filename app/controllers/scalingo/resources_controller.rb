class Scalingo::ResourcesController < ApplicationController
  ...
  def create
    ...
    render json: { id: 'uuid', message: 'Addon is being provisioned' }, status: 202
  end

  # Endpoint requested when the user resumes an addon
  # (after account suspension/end of free trial)
  ...
  def update
    ...
    if !params.key? 'plan'
      return render json: { error: 'update resource need a plan' }, status: 422
    end

    return render json: { message: 'Plan changed' }, status: 200
  end
  ...
  def addon_protected!
    if Rails.env != 'development' && !authenticate_with_http_basic { |u,p|
